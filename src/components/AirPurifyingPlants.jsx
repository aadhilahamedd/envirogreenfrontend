...
function AirPurifyingPlants() {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                const filtered = response.data.filter(plant => 
                    (plant.category || "").toLowerCase().includes("air-purifying plants")
                );
    ...
    return (
        <div className='row py-5 p-5 w-100'>
            <h2 className='text-center my-5 pe-5 me-5'>Air-Purifying Plants</h2>
            
            {viewPlant.length === 0 ? (
                <div className='text-center mt-5 w-100'>
                    <h4 className='text-muted'>No Air-Purifying Plants found in the database.</h4>
                </div>
            ) : (
