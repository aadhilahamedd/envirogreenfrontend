...
files.forEach(file => {
    ...
    // Replace <Button variant="primary">Add to Cart</Button>
    let idCounter = 1;
    // We need to dynamically wrap the button with onClick passing the hardcoded card info
    content = content.replace(/<Card[^>]*>[\s\S]*?<Card\.Img[^>]*src=["']([^"']+)["'][^>]*>[\s\S]*?<Card\.Title>([^<]+)<\/Card\.Title>[\s\S]*?<p[^>]*>Rs:(\d+)<\/p>[\s\S]*?<Button[^>]*>Add to Cart<\/Button>/g, (match, image, title, price) => {
